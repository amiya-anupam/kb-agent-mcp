...
What it does (in order):
  ...
  3.  Deletes agent_*.py files for folders that no longer exist
  ...
  7.  Generates agents/agent_<folder>.py for each domain
  8.  Regenerates agents/agent_knowledgebase.py (the orchestrator)
  9.  Writes requirements.txt
  10. Writes README.md
  11. Writes agents/SKILL.md and auto-copies to ~/.bob/skills/ if Bob is installed
...
#!/usr/bin/env python3
"""
{agent_filename} — {agent_display_name}
{separator}
Auto-generated by generate.py. Re-run generate.py to update.

Domain      : {folder_name}
Description : {description}
Agent file  : agents/{agent_filename}

Usage (standalone):
  python3 agents/{agent_filename} "your question"

Usage (called by orchestrator):
  from agents.{module_name} import ask
...
def ask(question: str, conversation_history: list[dict] | None = None) -> dict:
    """
    Answer a question using {folder_name}.
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(f'Usage: python3 agents/{agent_filename} "your question"')
        sys.exit(1)
...
def generate_agent_file(
    ...
    """Write agents/agent_<safe>.py for a single folder."""
    safe        = folder_to_safe_name(folder_name)
    fname       = f"agent_{safe}.py"
    module_name = f"agent_{safe}"
    display     = f"{folder_name} Agent"
    sep         = "-" * (len(fname) + len(display) + 4)

    system_prompt = (
        f"You are the {display}, a specialist in the {folder_name} knowledge domain.\n"
        f"Domain description: {description}\n"
    ...
    # Escape any triple-quotes in the system prompt
    system_prompt_escaped = system_prompt.replace('"""', '\\"\\"\\"')

    content = AGENT_TEMPLATE.format(
        agent_filename     = fname,
...
# ── Orchestrator note ─────────────────────────────────────────────────────────
# agent_knowledgebase.py is fully dynamic — it reads domain_meta.json at runtime
# and uses importlib to load sub-agents. It does NOT need to be regenerated.
# generate.py writes domain_meta.json (Step 6) and that is sufficient.
...
def main():
    ...
    # ── Step 2: delete orphaned agent .py files AND stale _index.json files ───
    expected_agent_files = {agent_filename(f) for f in folders} | {
        "agent_knowledgebase.py", "agent_base.py", "embeddings.py", "memory.py"
    }
    for existing_agent in agents_dir.glob("agent_*.py"):
        if existing_agent.name not in expected_agent_files:
            existing_agent.unlink()
            print(f"  🗑  Deleted orphaned agent: {existing_agent.name}")
    ...
    # ── Step 7: generate agent_<folder>.py files ──────────────────────────────
    print("Generating sub-agent files...")
    for d in domains_list:
        out = generate_agent_file(d["folder_name"], d["description"], agents_dir)
        print(f"  ✓ {out.name}")
    print()

    # ── Step 8: orchestrator is dynamic — domain_meta.json is all it needs ────
    # agent_knowledgebase.py reads domain_meta.json at runtime; no code gen needed.
    print("  ✓ Orchestrator ready (reads domain_meta.json at runtime)\n")
