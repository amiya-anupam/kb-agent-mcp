"""
_context_budget_logic.py — Pure compaction logic, shared by both context_budget modules
----------------------------------------------------------------------------------------
This module contains only pure logic: no env reads, no mutable globals.
All functions that need budgets take a `budgets` dict as their first argument.

Consumer modules (agents/context_budget.py, kb_agent_mcp/context_budget.py) import
these functions and create thin wrappers that bind their own `_BUDGETS` dict via
functools.partial, exposing the clean public API (no `budgets` param visible to callers).

Do NOT import this module directly from application code — use context_budget.py.
"""
...
def trim(budgets: dict[str, int], text: str, budget_key: str) -> str:
...
def trim_summary(budgets: dict[str, int], summary: str, filename: str = "") -> str:
...
def _collapse_label(
    budgets: dict[str, int], label: str, n: int, filenames: list[str]
) -> str:
    ...
    return trim_summary(budgets, desc)


# ── AUTO-INDEX block compaction ───────────────────────────────────────────────

def compact_index_block(budgets: dict[str, int], block: str) -> str:
    """
    ...
    if groups:
        ...
        for i, line in enumerate(new_lines):
            ...
            if matched_label not in emitted:
                n = len(info["files"])
                desc = _collapse_label(budgets, matched_label, n, info["files"])
    ...
    for line in lines:
        if _is_data_row(line):
            c = _cells(line)
            if len(c) >= 2:
                summary = trim_summary(budgets, c[1], _filename_from_row(line))
                line = f"| {c[0]} | {summary} |"
        out.append(line)
    lines = out
...
def compact_pre_index(budgets: dict[str, int], text: str) -> str:
    """Trim the hand-written README intro to KB_BUDGET_PRE_INDEX chars."""
    return trim(budgets, text.strip(), "pre_index")


# ── Final context assembly ────────────────────────────────────────────────────

def build_context(budgets: dict[str, int], pre_index: str, index_block: str) -> str:
    """Assemble the final context string within KB_BUDGET_INDEX chars."""
    pre = compact_pre_index(budgets, pre_index) if pre_index else ""
    index = compact_index_block(budgets, index_block)
    combined = (pre + "\n\n" + index) if pre else index
    return trim(budgets, combined, "index")
