...
folder and every nested subfolder. This is enforced in five independent places:

**MCP-layer (`kb_agent_mcp/`):**
...
**Agents-layer (`agents/` and `scripts/`):**
- **`agents/agent_base.extract_full_text()`** — returns a `[Excluded]` sentinel
  instead of file content for any file under a `.noindex` ancestor.
- **`agents/embeddings.should_skip()`** — `.noindex`-protected files are never
  added to the JSON vector index at build time.
- **`scripts/watch_kb.should_skip()`** — the filesystem watcher ignores
  `.noindex`-protected files; they are never re-embedded on change events.

Because all five enforcement points are independent, even if one were bypassed the
file would still be excluded by the remaining guards in both layers.
...
The `agents/` directory runs alongside `kb_agent_mcp/`. Both systems operate
independently from the same `KB_ROOT`.
...
**Security gate:** implemented only in `kb_agent_mcp/security_gate.py`. Legacy
`agents/` queries bypass the gate — this is intentional for backward compatibility
and does not affect MCP-based deployments.

**`.noindex` enforcement:** the agents-layer does enforce `.noindex` at both read
time (`agent_base.extract_full_text`) and index time (`embeddings.should_skip`,
`scripts/watch_kb.should_skip`). These guards were added alongside the MCP-layer
to close the bypass path described in the security model.
